import receitaData from './db.json';
import { Receita } from './dbStruct';

const db: Record<string, Receita> = receitaData;
...
function calculateBaseOutcome(rawMaterialNeeded: Record<string, number>, receitaFinalOutcome: Receita){
...
export function calculateMaximumEfficiency(idFinalOutcome: string) {
    const receitaFinalOutcome: Receita = db[idFinalOutcome];
    const rawMaterialNeeded: Record<string, number> = {};
    // IF IT IS A RAW MATERIAL, RETURNS ITSELF
    if (!receitaFinalOutcome) return formatToFrontend({ [idFinalOutcome]: 1 }, 1, idFinalOutcome);


    const materialsCraftingTree: connectionGraph[] = [];
